...
export KG_DAILY_TEMPLATE='<template-relative-to-KG_VAULT>'          # override discovery; seeds a brand-new daily note
...
# Machine label in the block heading (default: short hostname)
export KG_RECAP_HOST=laptop
```
...
When today's daily note does not exist yet, it is seeded from the vault's daily-note template before the block is written — the template path comes from the README via the same discovery step (`template:`), and the literal `{{date}}` placeholder is replaced with today's date. A vault whose README names no template keeps the previous behaviour: the file starts with the recap block. The template is never re-applied to a note that already exists.

The heading names the machine that wrote the block — `## Session 11:27〜12:10  @mac` — so a vault fed by more than one machine stays readable. The host is deliberately absent from the `kg-recap-sid:` marker, which is the upsert key: qualifying it would orphan every block already in the vault. Blocks written before `v0.23.0` pick the host up the next time the same session is upserted.
...
**Notes consulted today.** A session that reads a note in the vault consulted knowledge that already existed — a procedure written three months ago, say. The created-notes convention only covers notes born today, so re-use leaves no trace in the journal. When the vault README declares a `references_section` heading, each Stop appends one bullet per note read during the session, and two days that touched the same note become connected through that note instead of through a link between the days. Entries are mechanical (captured `Read` calls whose path is under `$KG_VAULT`, never a judgement about what mattered), append-only, and skipped when the link is already somewhere in the note — which both de-duplicates across sessions and drops whatever the created-notes section already lists. Reads outside the vault stay skipped as navigation noise, and with no heading declared nothing is written.
